...
## sample file ###############################
pruned_trees <- list.files(path="~/Desktop/Finished_tree_prunes", recursive=TRUE)
finished_musse <- list.files(path="~/Desktop/Finished_MUSSE", recursive=TRUE)
...
files <- paste0("~/Desktop/Finished_tree_prunes/", pre_files)
...
chew_musse <- function(phy, FRACTION_REMAINING = 1 , NUM_STATES=4, STEP_NUM=1000){
  ...
  flag<-runif(1)
	...
	return(list( MLE_answer.notAd, samples.notAd, stats_list, MLE_answer.pruneAdjusted, samples.pruneAdjusted))
}
...
musse_analysis_1a <- chew_musse(pruned_trees_and_stats[[2]], FRACTION_REMAINING = fraction_removed)
musse_analysis_2a <- chew_musse(pruned_trees_and_stats[[2]], FRACTION_REMAINING = fraction_removed)
musse_analysis_3a <- chew_musse(pruned_trees_and_stats[[3]], FRACTION_REMAINING = fraction_removed)
musse_analysis_4a <- chew_musse(pruned_trees_and_stats[[4]], FRACTION_REMAINING = fraction_removed)
musse_analysis_5a <- chew_musse(pruned_trees_and_stats[[5]], FRACTION_REMAINING = fraction_removed)
...
save(pruned_trees_and_stats_and_musse, file=paste("~/Desktop/Finished_MUSSE/", pre_files, sep=""))
